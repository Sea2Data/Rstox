...
baseline2ECA <- function(projectName, biotic="BioticCovData", landing="LandingCovData", temporal=NULL, gearfactor=NULL, spatial=NULL){
	...
	if(all(c(biotic[1], landing[1]) %in% names(baselineOutput$out))){
		...
		# Get the covariates in their raw form_
		covariateMatrixBioticRaw <- biotic[covariateNames]
		covariateMatrixLandingRaw <- landing[covariateNames]
		...
		list(biotic=biotic, landing=landing, landingAggregated=landingAggregated, covariateMatrixBiotic=covariateMatrixBiotic, covariateMatrixBioticRaw=covariateMatrixBioticRaw, covariateMatrixLanding=covariateMatrixLanding, covariateMatrixLandingRaw=covariateMatrixLandingRaw, resources=list(covariateInfo=covariateInfo, covariateDefinition=covariateDefinition, covariateLink=covariateLink, ageError=ageErrorMatrix, stratumNeighbour=stratumNeighbourList))
	}
	else{
...
baseline2eca <- function(projectName, biotic="BioticCovData", landing="LandingCovData", temporal=NULL, gearfactor=NULL, spatial=NULL){
	...
	if(all(c(biotic[1], landing[1]) %in% names(baselineOutput$out))){
		...
		# Return all data in a list
		list(biotic=biotic, landing=landing, landingAggregated=landingAggregated, covariateMatrixBiotic=covariateMatrixBiotic, covariateMatrixLanding=covariateMatrixLanding, resources=list(covariateInfo=covariateInfo, covariateDefinition=covariateDefinition, covariateLink=covariateLink, ageError=ageErrorMatrix, stratumNeighbour=stratumNeighbourList))
	}
	else{
		warning(paste0("The processes ", paste(c(biotic[1], landing[1]), collapse=" and "), "does not exist in the baseline model"))
		invisible(NULL)
