	...
	if(any(c("transect", "transects") %in% tolower(plot))){
		...
		# Use the retour as line type:
			...
			if(length(timerange) == 2){
				insideTimeRange <- x$Transect$time_mid >= timerange[1] & x$Transect$time_mid <= timerange[2]
				x$Transect <- x$Transect[insideTimeRange, ]
			}
...
#'
#' \code{writeTransectsMaxSea} writes the transects of all strata to separate .asc files for import in MaxSea. \cr \cr
...
#'
writeTransects <- function(x, projectName=NULL, dir=NULL, digits=5, byStratum=TRUE, cols=NULL, keepTransport=TRUE, prefix="", suffix="", filenames=NULL, ext="txt", ...){
...
#' @rdname writeTransects
#' 
writeTransectsMaxSea <- function(x, projectName=NULL, dir=NULL, item.type=257, item.id=20, item.col=1, north="N", east="E", digits=3, prefix="", suffix="", filenames=NULL, ...){
	# Function for writing one stratum:
	...
	x$Transect$abs_lat_start <- abs(x$Transect$lat_start)
	x$Transect$abs_lon_start <- abs(x$Transect$lon_start)
...
#' @rdname writeTransects
#' 
writeTransectsINFO <- function(x, projectName=NULL, dir=NULL, digits=2, prefix="", suffix="", filenames=NULL){
	# Function for writing one stratum:
...
#' @rdname writeTransects
#' 
writeTransectsTRACK <- function(x, projectName=NULL, dir=NULL, digits=5, prefix="", suffix="", filenames=NULL, ...){
	# Function for writing one stratum:
...
#' @rdname writeTransects
#' 
writeTransectsGPX <- function(x, projectName=NULL, dir=NULL, digits=5, prefix="", suffix="", filenames=NULL){
	# Function for writing one stratum:
